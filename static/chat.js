...
function resolveAgent(name) {
    ...
    for (const [key, cfg] of Object.entries(agentConfig)) {
        const command = (cfg.command || '').toLowerCase();
        const label = (cfg.label || '').toLowerCase();
        if (s === command || s === label) return key;
    }
    return null;
}
...
function getDisplayName(sender) {
    ...
    const base = sender.toLowerCase().replace(/-\d+$/, '');
    if (baseColors[base] && baseColors[base].label) return baseColors[base].label;
    return sender;
}
